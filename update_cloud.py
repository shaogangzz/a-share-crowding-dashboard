...
def east_json(url,params):
    ...
    for n in range(6):
        try:
            r=HTTP.get(url,params=params,timeout=(10,40))
            if r.status_code!=200: raise RuntimeError("HTTP "+str(r.status_code))
            text=r.text.lstrip()
            if not text or text[0] not in "{[": raise RuntimeError("non-json response: "+text[:100])
            return r.json()
        except Exception as e:
            last=e; time.sleep(1.2+n*1.2+random.random())
    raise last
...
def fetch_rows():
    ...
    for host in ["https://push2.eastmoney.com/api/qt/clist/get","https://push2delay.eastmoney.com/api/qt/clist/get"]:
        ...
        except Exception as e: errors.append(host+" "+repr(e)[:160])
    try:
        ...
        for n in range(3):
            ...
            except Exception as e: errors.append("AKShare#"+str(n+1)+" "+repr(e)[:160]); time.sleep(3+n*2)
...
def backfill(rows, target_names, days=260):
    """一次性补齐目标行业约一年交易日；失败行业返回名单，调用方会在同一次任务中重试。"""
    end=datetime.date.today().strftime("%Y%m%d"); wanted=set(target_names); allrows=[]; ok=[]; failed=[]
    for r in rows:
        name=str(r.get("name",""))
        if name not in wanted: continue
        code=str(r.get("code",""))
        if not code.startswith("BK"): failed.append(name); continue
        try:
            j=east_json("https://push2his.eastmoney.com/api/qt/stock/kline/get",{
                "secid":"90."+code,"fields1":"f1,f2,f3,f4,f5,f6","fields2":"f51,f52,f53,f54,f55,f56,f57,f58,f59,f60,f61",
                "klt":"101","fqt":"0","end":end,"lmt":days,"_":int(time.time()*1000)})
            klines=j.get("data",{}).get("klines",[])
            if len(klines)<200: raise RuntimeError("历史K线不足200日")
            count=0
            for line in klines:
                p=line.split(",")
                if len(p)>=11:
                    allrows.append({"date":p[0],"name":name,"close":num(p[2]),"pct":num(p[8]),"amount":num(p[6]),"turnover":num(p[10]),"pe":np.nan,"pb":np.nan}); count+=1
            if count>=200: ok.append(name)
            else: failed.append(name)
        except Exception: failed.append(name)
        time.sleep(.18+random.random()*.18)
    return pd.DataFrame(allrows),ok,failed
...
def save_result(result): OUT.write_text(json.dumps(result,ensure_ascii=False,indent=2,allow_nan=False),encoding="utf-8")
...
try:
    ...
    for name,g in hist.groupby("name"):
        ...
        def hr(s,minn=20):
            s=pd.to_numeric(pd.Series(s),errors="coerce").dropna(); return s.rank(pct=True).iloc[-1]*100 if len(s)>=minn else np.nan
    ...
    # ===== 一次性彻底补齐历史：不再按20个行业慢慢补 =====
    series=[]
    target_names=[str(r["name"]) for r in rows]
    # 直接重建历史序列，确保所有行业使用同一套约一年数据和同一套横截面评分口径
    rawdf=pd.DataFrame(); ok=[]; failed=[]
    pending=target_names[:]
    for attempt in range(3):
        if not pending: break
        part,got,bad=backfill(rows,pending,days=260)
        if not part.empty: rawdf=pd.concat([rawdf,part],ignore_index=True)
        ok=sorted(set(ok)|set(got)); pending=sorted(set(bad))
        if pending: time.sleep(2+attempt*3)
    failed=sorted(set(pending))
    if rawdf.empty or len(ok)<max(20,int(len(target_names)*0.7)):
        raise RuntimeError(f"历史数据初始化不完整：成功{len(ok)}/{len(target_names)}，失败{len(failed)}")

    scored=historical_scores(rawdf.drop_duplicates(["name","date"],keep="last").sort_values(["date","name"]))
    ...
    cov=coverage_map(series); complete=sum(1 for n in target_names if cov.get(n,0)>=200); missing=len(target_names)-complete
    top=[x["name"] for x in out[:3]]; opps=sorted(out,key=lambda x:x["opportunity"],reverse=True)[:3]
    quality=f"五因子实时评分｜历史数据完整度 {complete}/{len(target_names)} 个行业≥200交易日｜本次初始化成功 {len(ok)}/{len(target_names)}｜数据源：{source}"
    if failed: quality+=f"｜失败{len(failed)}个（已重试3轮）"
    result={"date":today_date,"status":"云端自动更新成功" if not failed else "云端更新成功，少量行业待重试","data_quality":quality,
            "summary":"当前拥挤度较高："+"、".join(top)+"。机会雷达关注："+"、".join(x["name"] for x in opps)+"。"+(f"仍有{missing}个行业历史数据不足，将自动重试。" if missing else "全部行业历史数据已完成初始化。"),
            "industries":out,"history_points":len(series),"history_progress":{"complete":complete,"total":len(target_names),"missing":missing,"failed":failed}}
    save_result(result); print(result["status"],len(out),len(series),"complete",complete)
except Exception as e:
    if previous and previous.get("industries"):
        previous["status"]="本次更新失败，展示上一份有效数据｜"+repr(e)[:120]; previous["data_quality"]=(previous.get("data_quality") or "")+"｜已启用数据保护"; save_result(previous); print(previous["status"])
    else:
        result={"date":today_date,"status":"数据源暂时不可用，系统将在下次自动任务重试："+repr(e)[:160],"data_quality":"等待首次成功抓取","summary":"本次未获得有效行情数据，未生成虚假数据。","industries":[],"history_points":0}; save_result(result); print(result["status"])
